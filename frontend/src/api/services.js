...
export const authApi = {
  ...
  profile: () =>
    http.get('/auth/profile').then((r) => r.data),
  ...
  searchUsers: (q) =>
    http
      .get(`/auth/users/search?q=${encodeURIComponent(q)}`)
      .then((r) => r.data),
};

export const repoApi = {
  list: (page = 1, limit = 20) =>
    http.get(`/repos?page=${page}&limit=${limit}`).then((r) => r.data),

  create: (payload) =>
    http.post('/repos', payload).then((r) => r.data),

  detail: (id) =>
    http.get(`/repos/${id}`).then((r) => r.data),

  delete: (id) =>
    http.delete(`/repos/${id}`).then((r) => r.data),

  addCollaborator: (id, payload) =>
    http.post(`/repos/${id}/collaborators`, payload).then((r) => r.data),

  removeCollaborator: (id, userId) =>
    http.delete(`/repos/${id}/collaborators/${userId}`).then((r) => r.data),

  updateCollaboratorRole: (id, userId, payload) =>
    http.put(`/repos/${id}/collaborators/${userId}`, payload).then((r) => r.data),

  issues: (id, params = {}) => {
    const searchParams = new URLSearchParams(params);

    return http
      .get(`/repos/${id}/issues?${searchParams.toString()}`)
      .then((r) => r.data);
  },

  createIssue: (id, payload) =>
    http.post(`/repos/${id}/issues`, payload).then((r) => r.data),

  activity: (id, page = 1, limit = 20) =>
    http
      .get(`/api/repos/${id}/activity?page=${page}&limit=${limit}`)
      .then((r) => r.data),

  archive: (id) =>
    http.put(`/repos/${id}/archive`).then((r) => r.data),

  labels: (id) =>
    http.get(`/repos/${id}/labels`).then((r) => r.data),

  createLabel: (id, payload) =>
    http.post(`/repos/${id}/labels`, payload).then((r) => r.data),

  deleteLabel: (id, labelId) =>
    http.delete(`/repos/${id}/labels/${labelId}`).then((r) => r.data),
};

export const issueApi = {
  comments: (issueId, page = 1, limit = 20) =>
    http
      .get(`/issues/${issueId}/comments?page=${page}&limit=${limit}`)
      .then((r) => r.data),
  ...
  close: (issueId) =>
    http.put(`/issues/${issueId}/close`).then((r) => r.data),

  reopen: (issueId) =>
    http.put(`/issues/${issueId}/reopen`).then((r) => r.data),

  update: (issueId, payload) =>
    http.put(`/issues/${issueId}`, payload).then((r) => r.data),
...
export const prApi = {
  list: (repoName) =>
    http.get(`/api/pr/repo/${repoName}`).then((r) => r.data),

  detail: (prId) =>
    http.get(`/api/pr/item/${prId}`).then((r) => r.data),

  create: (payload) =>
    http.post('/api/pr', payload).then((r) => r.data),

  merge: (prId) =>
    http.put(`/api/pr/${prId}/merge`).then((r) => r.data),

  close: (prId) =>
    http.put(`/api/pr/${prId}/close`).then((r) => r.data),

  reopen: (prId) =>
    http.put(`/api/pr/${prId}/reopen`).then((r) => r.data),

  submitReview: (prId, payload) =>
    http.post(`/api/pr/${prId}/review`, payload).then((r) => r.data),

  comments: (prId) =>
    http.get(`/api/review/${prId}`).then((r) => r.data),

  addComment: (payload) =>
    http.post('/api/review/comment', payload).then((r) => r.data),
...
export const gitApi = {
  commits: (repoName, page = 1, limit = 20) =>
    http
      .get(`/api/git/repos/${repoName}/commits?page=${page}&limit=${limit}`)
      .then((r) => r.data),
...
export const notificationApi = {
  ...
  count: () =>
    http.get('/api/notifications/count').then((r) => r.data),

  read: (id) =>
    http.patch(`/api/notifications/${id}/read`).then((r) => r.data),

  readAll: () =>
    http.put('/api/notifications/read-all').then((r) => r.data),
};
