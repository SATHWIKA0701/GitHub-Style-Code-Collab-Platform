...
export const deleteRepo = asyncHandler(async (req, res) => {
  ...
  if (!repo) return res.status(404).json({ message: "Repository not found" });
  if (String(repo.owner) !== String(req.user.id)) return res.status(403).json({ message: "Not authorized" });
...
export const addCollaborator = asyncHandler(async (req, res) => {
  ...
  if (!["owner", "collaborator", "viewer"].includes(requestedRole)) return res.status(400).json({ message: "Invalid role" });
  if (requestedRole === "owner") return res.status(400).json({ message: "Use ownership transfer to set owner" });
  if (String(repo.owner) === String(userId)) return res.status(400).json({ message: "User is already the owner" });
