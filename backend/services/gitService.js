...
import { withRepoLock } from "../utils/repoLock.js";
...
const assertSafeCommitSha = (sha) => {
  const value = typeof sha === "string" ? sha.trim() : "";
...
const ensureGitIdentity = async (git, user = null) => {
  const name =
    user?.username ||
    user?.name ||
  ...
  const email =
    ...
    "noreply@code-collab-platform.local";
...
export const initRepository = async (repoName, defaultBranch = "main") =>
  withRepoLock(repoName, async () => {
    ...
    if (!branchInfo.all.includes(defaultBranch)) {
...
export const commitFiles = async (repoName, message, user = null) =>
  withRepoLock(repoName, async () => {
    const safeMessage =
...
export const getStructuredCommits = async (repoName) => {
  ...
  return log
    ...
    .split("\n")
    .map((line) => {
      const [hash, parents, subject, authorName, authorEmail, date] =
        line.split("|");
...
export const getCommitDetails = async (repoName, sha) => {
  const safeSha = assertSafeCommitSha(sha);
  const git = simpleGit(getRepoPath(repoName, true));

  return await git.show(["--stat", "--patch", safeSha]);
};
...
export const writeFileContent = async (repoName, filePath, content) =>
  withRepoLock(repoName, async () => {
    const repoPath = getRepoPath(repoName, true);
    const safeRelativePath = assertSafeRelativePath(filePath);
  ...
  withRepoLock(repoName, async () => {
    ...
    for (const file of files) {
      ...
      uploaded.push({
        ...
        path: [safeDirectory, file.originalname].filter(Boolean).join("/"),
      });
    }
...
export const createFolder = async (repoName, relativePath) =>
  withRepoLock(repoName, async () => {
    ...
    fs.mkdirSync(path.join(repoPath, safeRelativePath), {
      recursive: true,
    });
